...
public class PhantomChargeRenderer extends EntityRenderer<PhantomCharge, EntityRenderState> {
    private static final Identifier TEXTURE_LOCATION = ProjectSanguine.id("textures/entity/projectiles/phantom_charge.png");
    private static final RenderType RENDER_TYPE = RenderTypes.entityCutout(TEXTURE_LOCATION);

    public PhantomChargeRenderer(EntityRendererProvider.Context context) {
        super(context);
    }

    protected int getBlockLightLevel(final PhantomCharge entity, final BlockPos blockPos) {
        return 15;
    }
